...
//	        page.setCount(Long.valueOf(createQuery(countQlString, parameter).uniqueResult().toString()));
	...
	protected T mergeOrUpdate(T entity, String id) {
...
//			getSession().merge(entity);
	 ...
	 * @return
	 */
	public int deleteByIdPhysic(Serializable id){
		return update("delete "+entityClass.getSimpleName()+" where  id=:p1 ", new Parameter(id));	
	}
	 ...
	 * @throws Exception 
		 */
		public <T> Page<T> findBySqlExt(Page<T> page, String sqlString, Class<?> resultClass) {
	    	return findBySqlExt(page, sqlString, null, resultClass);
	    }
	 ...
	 */
    @SuppressWarnings("unchecked")
	public <T> Page<T> findBySqlExt(Page<T> page, String sqlString, Parameter parameter, Class<?> resultClass) {
		// get count
    	if (!page.isDisabled() && !page.isNotCount()){
...
//	        page.setCount(Long.valueOf(createSqlQuery(countSqlString, parameter).uniqueResult().toString()));
        ...
        List<T> result;
		try {
			result = this.transBeanList(list, resultClass);
			page.setList(result);
    ...
    /***
     * 获取总数
     * @param countSqlString
     * @param parameter
     * @return
     */
    public long getCount(String countSqlString, Parameter parameter){
        Query query = createSqlQuery(countSqlString, parameter);
        List<Object> list = query.list();
        if (list.size() > 0){
	...
	@SuppressWarnings("unchecked")
	public <E> E findFirstBySQL(String sqlString, Parameter parameter, Class<?> resultClass) throws Exception{
		SQLQuery query = createSqlQuery(sqlString, parameter);
		//setResultTransformer(query, resultClass);		
        setResultTransformer(query, Map.class);
        List<Map> list = query.list();        
        List<E> result = transBeanList(list, resultClass);
...
//	        page.setCount(Long.valueOf(createSqlQuery(countSqlString, parameter).uniqueResult().toString()));
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <E> List<E> findBySqlExt(String sqlString, Parameter parameter, Class<?> resultClass) {
		SQLQuery query = createSqlQuery(sqlString, parameter);
		//setResultTransformer(query, resultClass);		
        setResultTransformer(query, Map.class);
        List<Map> list = query.list();        
        List<E> result;
		try {
			result = transBeanList(list, resultClass);
			return result;
		} catch (InstantiationException e) {
			throw new RuntimeException(e);
		} catch (IllegalAccessException e) {
     ...
     * @return
     */
    private boolean isExistsOrderBy(String qlString){
        Pattern p = Pattern.compile("order\\s*by[\\w|\\W|\\s|\\S]*", Pattern.CASE_INSENSITIVE);  
        Matcher m = p.matcher(qlString);  
        StringBuffer sb = new StringBuffer();  
        if (m.find()) {  
	 ...
	 * @return
	 */
	public int getNextValue(String sequenceNm){	
		String sql = "select _nextval('"+sequenceNm+"')";
	 ...
	 * @throws InvocationTargetException 
	 */
	protected <T> List<T> transBeanList(List<Map> list ,Class<?> resultClass ) throws InstantiationException, IllegalAccessException, ClassNotFoundException, InvocationTargetException{
		if(list == null)
			return Lists.newArrayList();
		
		List<T> result = Lists.newArrayList();
		String classNm = resultClass.getName();
		T entity=null;				
		for(Map map: list){
			entity = (T)Class.forName(classNm).newInstance();
			for (Object key : map.keySet()) {
				String feildNm = key.toString();
				Object feildValue = map.get(key);
				feildNm = StringUtils.spellField(feildNm);
				if(feildValue != null)
					BeanUtils.setProperty(entity, feildNm, feildValue);						
			}			
			result.add(entity);
		}
		return result;
