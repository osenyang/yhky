...
<body>
	<form:form id="searchForm" modelAttribute="area" action="${ctx}/sys/dict/" method="post" class="breadcrumb form-search">
		<shiro:hasPermission name="sys:area:edit"><a href="${ctx}/sys/area/form"><input id="btnSubmit" class="btn btn-yellow" type="button" value="区域添加"/></a></shiro:hasPermission>
	</form:form>
		...
		<c:forEach items="${list}" var="area">
			<tr id="${area.id}" pId="${area.parent.id ne requestScope.area.id?area.parent.id:'0'}">
				...
				<td>					
					...
					<c:if test="${area.type  == '4'}">
						<a href="${ctx}/sys/community/form?areaId=${area.id}">添加社区</a>
					</c:if>
				</td>
				</shiro:hasPermission>
			</tr>
